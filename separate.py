...
def run(args):
    ...
    elif config_dict["model_type"] == "U_Conformer_v23":
        config_dict["model"]["crm"] = config_dict['crm']
        config_dict["model"]["IPD_sincos"] = dataloader_conf['IPD_sincos']
        nnet = U_ConformerCSS_v23(**config_dict["model"])
    ...
    for key, utt in utt_dict.items():
        ...
        if CSS_config["CSS"]:
            ...
            spk_spectrogram = [(stft_mat[:,:,0] if len(stft_mat.shape) == 3 else stft_mat) * spk_mask[:-dummy_frame_len] 
                               for spk_mask in mask_concat]
        else:
