...
class PITrainer_RI(object):
    ...
    def train(self, dataset, epoch):
        ...
        tot_loss_s = tot_loss_s_spec = tot_loss_n = num_batch = 0
        ...
        for input_sizes, mixture, target_rir, target, noise in dataset:
            ...
            cur_loss_s = self.PIT_loss(masks[:self.num_spks], input_sizes, source_attr, target_attr)
            tot_loss_s_spec += cur_loss_s_spec.item() / self.num_spks
            tot_loss_s += cur_loss_s.item() / self.num_spks
            # print(cur_loss_s.item() / self.num_spks)

            if len(masks) == self.num_spks:
                cur_loss = (0.3 * cur_loss_s_spec + 0.7 * cur_loss_s) / self.num_spks
            else:
                ...
                cur_loss = (0.3 * cur_loss_s_spec + 0.7 * cur_loss_s + cur_loss_n) / (self.num_spks + 1)            
            ...
            if self.clip_norm:
                th.nn.utils.clip_grad_norm_(self.nnet.parameters(),
                                            self.clip_norm)
            self.optimizer.step()
            pbar.set_postfix({'Loss': tot_loss_s/num_batch, 'Loss_spec': tot_loss_s_spec/num_batch, 'Loss_n': tot_loss_n/num_batch})
        ...
        def A_SDR_loss(permute, eps=1.0e-6):
            ...
            for s, t in enumerate(permute):
                if masks == None:
                    ...
                    x = self.inverse_stft(masks[s].abs()*mixture_spect, masks[s].angle()+mixture_phase, cplx=False)                    
