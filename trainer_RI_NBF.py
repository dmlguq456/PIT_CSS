...
class PITrainer_RI_NBF(object):
    ...
    def train(self, dataset, epoch):
        ...
        tot_loss_s_time = tot_loss_s_freq = tot_loss_n = num_batch = 0
        ...
        for input_sizes, mixture, target_rir, target, noise, degs_diff, DOAs in dataset:
            ...
            outputs = th.nn.parallel.data_parallel(self.nnet, nnet_input, device_ids=self.gpuid)
            cur_loss_s_time = self.PIT_loss(outputs[:self.num_spks], input_sizes, target_attr)
            cur_loss_s_freq = self.PIT_loss_spec(input_sizes, outputs[:self.num_spks],  target_attr)
            tot_loss_s_time += cur_loss_s_time.item() / self.num_spks
            tot_loss_s_freq += cur_loss_s_freq.item() / self.num_spks
            # print(cur_loss_s.item() / self.num_spks)

            if len(outputs) == self.num_spks:
                cur_loss = (0.5*cur_loss_s_time + 0.5*cur_loss_s_freq) / self.num_spks
            else:
            ...
            if self.clip_norm:
                th.nn.utils.clip_grad_norm_(self.nnet.parameters(),
                                            self.clip_norm)
            self.optimizer.step()
            pbar.set_postfix({'Loss_T': tot_loss_s_time/num_batch,'Loss_F': tot_loss_s_freq/num_batch})
           # th.cuda.empty_cache() # personal edit
        pbar.close()
        return tot_loss_s_time / num_batch, tot_loss_s_freq / num_batch, num_batch
    ...
    def run(self, train_set, dev_set, num_epoches=20, log_dir='log'):
        with th.cuda.device(self.gpuid[0]):
            ...
            for epoch in range(self.start_epoch, num_epoches + self.start_epoch):
                ...
                train_loss_src_time, train_loss_src_freq, train_num_batch = self.train(train_set, epoch)
                on_valid_start = time.time()
                valid_loss_src_time, valid_loss_src_freq, valid_num_batch = self.validate(dev_set)
                on_valid_end = time.time()
                # scheduler learning rate
                self.scheduler.step(valid_loss_src_time)
                logger.info(
                    "Loss(time/mini-batch) \n - Epoch {:2d}: train for source = {:.4f} for time and {:.4f} for freq ({:.2f}s/{:d}) |"
                    " dev for source = {:.4f} for time and {:.4f} for freq ({:.2f}s/{:d})".format(
                        epoch, train_loss_src_time, train_loss_src_freq, on_valid_start - on_train_start, train_num_batch, \
                        valid_loss_src_time, valid_loss_src_freq, on_valid_end - on_valid_start, valid_num_batch))
                writer_src.add_scalars("Loss_src",{'train_time':train_loss_src_time, 
                                                   'train_freq':train_loss_src_freq, 
                                                   'valid_time':valid_loss_src_time, 
                                                   'valid_freq':valid_loss_src_freq}, epoch)
                writer_src.flush()
                if valid_loss_src_time < valid_loss_best:
                    valid_loss_best = valid_loss_src_time
                    ...
                    th.save(
                            ...
                            'loss' : valid_loss_src_time
                            },
                            save_path)
        logger.info("Training for {} epoches done!".format(num_epoches))

    def PIT_loss(self, outputs, input_sizes, target_attr):
        ...
        def SA_SDR_loss(permute, eps=1.0e-6):
            ...
            for s, t in enumerate(permute):
                x = self.inverse_stft(outputs[s].abs(), outputs[s].angle(), cplx=False)
                s = self.inverse_stft(targets_spect[t], targets_phase[t], cplx=False)                
                x_zm = x - th.mean(x, dim=-1, keepdim=True)
                s_zm = s - th.mean(s, dim=-1, keepdim=True)
    ...
    def PIT_loss_spec(self, input_sizes, source_attr, target_attr):
    
        input_sizes = input_sizes.to(self.device)
        output_spect = source_attr
        targets_spect = [t.to(self.device) for t in target_attr["spectrogram"]]

        def SA_STFT_Mag_SDR_loss(permute, eps=1.0e-6):
            src_SA = mix_SA = th.tensor([]).to(self.device)
            for s, t in enumerate(permute):
                mix = output_spect[s].abs()
                src = targets_spect[t]
