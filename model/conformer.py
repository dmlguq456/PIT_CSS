...
class ConformerCSS(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_bias = torch.zeros(1,1,self.num_mics*in_features)
        self.input_scale = torch.ones(1,1,self.num_mics*in_features)
        ...
        # Conformer Encoders
        self.conformer = ConformerEncoder(self.num_mics*in_features, **conformer_conf)
        self.num_bins = num_bins
        num_nois = 1 if noise_flag else 0
        self.num_nois = num_nois
        self.in_features = in_features
        self.linear = nn.Linear(conformer_conf["attention_dim"], num_bins * (num_spks + num_nois) * (2 if self.crm else 1))
        self.relu = nn.ReLU()
