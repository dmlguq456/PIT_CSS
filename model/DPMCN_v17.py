...
class Dual_Path_Layer(nn.Module):
    ...
    def __init__(self, d_model_F, d_model_C, n_head_F, n_head_C, d_ffn_F, d_ffn_C, kernel_size, dropout_rate, causal=False, N_intra=1, N_inter=1, GRN_opt=True, pooling_ratio=8):
        """Construct an EncoderLayer object."""
        ...
        self.spatial_pool = nn.Linear(d_model_C, d_model_C//self.pooling_ratio)
        self.spatial_up = nn.Linear(d_model_C//self.pooling_ratio, d_model_C)
        self.spectral_pool = nn.Linear(d_model_F, d_model_F//self.pooling_ratio)
        self.spectral_up = nn.Linear(d_model_F//self.pooling_ratio, d_model_F)
    ...
    def forward(self, x, mask):
        ...
        x_intra = self.spatial_pool(x_intra)
        # x_intra = torch.nn.functional.adaptive_avg_pool1d(x_intra, C//self.pooling_ratio)
        x_intra = x_intra.view(B, T, F, C//self.pooling_ratio).permute(0, 1, 3, 2).contiguous()
        for layer in self.intra_block:
            x_intra = layer(x_intra, mask)

        x_intra = x_intra.permute(0, 1, 3, 2).contiguous().view(B*T, F, C//self.pooling_ratio)
        x_intra = self.spatial_up(x_intra)
        ...
        # x_inter = torch.nn.functional.adaptive_avg_pool1d(x_inter, F//self.pooling_ratio)
        x_inter = self.spectral_pool(x_inter)
        x_inter = x_inter.view(B, T, C, F//self.pooling_ratio)

        for layer in self.inter_block:
            x_inter = layer(x_inter, mask)

        x_inter = x_inter.view(B*T, C, F//self.pooling_ratio)
        x_inter = self.spectral_up(x_inter)
...
class DPConformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.embed_inter1 = torch.nn.Sequential(
            ...
            torch.nn.Linear(num_mic*(num_mic+1), attention_dim_C),
        ...
        self.DP_conformer = torch.nn.Sequential(
            *[Dual_Path_Layer(attention_dim_F, attention_dim_C, attention_heads_F, attention_heads_C, linear_units_F, linear_units_C, kernel_size, dropout_rate, causal=causal, N_intra=N_intra, N_inter=N_inter, GRN_opt=GRN_opt, pooling_ratio=pooling_ratio)
