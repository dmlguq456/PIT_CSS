...
class FrequenceyWiseLinear(nn.Module):
    def __init__(self, num_bins, in_ch, out_ch):
        super().__init__()
        self.freq_linear = nn.Parameter(torch.rand(1, num_bins, in_ch, out_ch))
...
class AttentiveChannelPool(nn.Module):
    def __init__(self, num_bins, num_ch, num_mics, reduct_ratio):
        super().__init__()
        
        self.freq_ffn_1 = nn.Linear(num_bins, num_bins//reduct_ratio)
        self.relu = nn.ReLU()
        self.ch_ffn = FrequenceyWiseLinear(num_bins//reduct_ratio, num_ch, 2*num_mics*num_ch)
    ...
    def forward(self, x):
        ...
        B, F, D = y.shape
        y = y.view(B, F, -1, C)
        y = y.permute(0, 2, 3, 1).contiguous() # B, M, C, F

        y = self.freq_ffn_2(y)
        y = y.permute(0, 3, 2, 1).contiguous() # B, F, C, M
        y = self.ch_softmax(y)
        
        y = x.unsqueeze(-1) * y.unsqueeze(1) # B, T, F, C, M
...
class DPConformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_estim_ch = AttentiveChannelPool(idim, attention_dim_C, num_mic, 4)
    ...
    def forward(self, x, masks):
        ...
        xs, x = self.get_scm(x_r.permute(0, 3, 2, 1), x_i.permute(0, 3, 2, 1)) # [B, T, 257, M*M]
        xs_abs = xs.abs()
        beta_phat = xs_abs**self.sigmoid(self.exponent)
       ...
       # [ B, F, T ]
        ...
        # C = xs.shape[-1]
        xs = xs.view(B, N, T, F, M)
        xs = torch.chunk(xs, 2, -1)
        xs = torch.complex(xs[0], xs[1])
        return xs, x
...
class DPMCN_v15_NBF(nn.Module):
    ...
    def __init__(self,
        ...
        # Conformer Encoders
        self.conformer = DPConformer(self.num_mics, self.num_spks, in_features, **conformer_conf)
    ...
    def forward(self, f, angle_dif=None, train=True):
        ...
        if len(f.shape) == 3:
            f = f.unsqueeze(0)
        w, x = self.conformer(f, masks=None) # B, N, T, F, M  / B, T, F, M
        y = torch.einsum('...fmt,...tfm->...tf', (x.unsqueeze(1), w)) # B, N, T, F
        y = y.permute(0, 1, 3, 2).contiguous()
        # m = m.transpose(1, 2)
        # B x T x F => B x F x T
        y = y.permute(0, 2, 3, 1).contiguous()
        y = torch.chunk(y, self.num_spks, -1) # B, T, F
        y = [y_e.squeeze(-1) for y_e in y]
