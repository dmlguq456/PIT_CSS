...
class DPConformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.mask_estim_real = torch.nn.Sequential(
            AttentiveChannelPool(idim, attention_dim_C, 4),
            nn.Linear(idim, idim)
        )
        self.mask_estim_imag = torch.nn.Sequential(
            AttentiveChannelPool(idim, attention_dim_C, 4),
            nn.Linear(idim, idim)
        )
    ...
    def forward(self, x, masks):
       ...
       # [ B, F, T ]
        ...
        m_real = self.mask_estim_real(xs)
        m_imag = self.mask_estim_imag(xs)
        # xs = self.mask_activation(xs)
        # C = xs.shape[-1]
        m_real = m_real.view(B, N, T, F).permute(0, 2, 1, 3).contiguous()
        m_real = m_real.view(B, T, N*F).contiguous()

        m_imag = m_imag.view(B, N, T, F).permute(0, 2, 1, 3).contiguous()
        m_imag = m_imag.view(B, T, N*F).contiguous()
...
class DPMCN_v16_CM(nn.Module):
    ...
    def forward(self, f, angle_dif=None, train=True):
        ...
        if len(f.shape) == 2:
            f = f.unsqueeze(0)
        m_r, m_i = self.conformer(f, masks=None)
