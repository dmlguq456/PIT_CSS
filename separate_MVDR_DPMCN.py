...
def masks_similarity(pre_mask, cur_mask, permute):
    ...
    for s, t in enumerate(permute):
        corr += - np.mean(abs(pre_mask[s] - cur_mask[t]))
    return corr, permute
...
def run(args):
    ...
    separator = Separator(nnet, config_dict["inference"]["check_point_dir"], config_dict["crm"], cuda=args.cuda, gpu_id=args.gpu_id)
        ...
        if CSS_config["CSS"]:
            ...
            for idx, (whole_begin, whole_end) in enumerate(chunk_list):
                ...
                spk_mask, spk_spect, spk_mvdr = separator.seperate(
                    stft_mat_chunk, mvn=mvn, apply_log=apply_log, cos_sin_opt=config_dict['trainer']['IPD_sincos'])
                if idx == 0:
            ...
            mask_mean = mask_concat[:,:-dummy_frame_len,:]
            mask_mean = mask_mean * (mask_mean > 0.2)
            mask_mean = mask_mean.mean(-1)
